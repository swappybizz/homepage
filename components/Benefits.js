...
const benefitsData = [ { id: 1, title: 'Enhance Compliance', description: 'Meet HSE and industry regulations effortlessly with standardized, verifiable digital audit trails. Reduce non-compliance risks.', }, { id: 2, title: 'Increase Accuracy & Consistency', description: 'Minimize human error and ensure consistent audit procedures across all technicians and sites with AI assistance and guided workflows.', }, { id: 3, title: 'Reduce Costs & Time', description: 'Significantly cut down on travel time, repeat visits, and manual reporting efforts. Faster audits mean quicker project turnaround.', }, { id: 4, title: 'Improve Data Security', description: 'End-to-end encryption and secure cloud storage protect sensitive audit data, ensuring privacy and integrity.', }, { id: 5, title: 'Faster Decision Making', description: 'Get real-time insights and generate reports instantly, enabling quicker responses to issues and informed strategic decisions.', }, ];
const colors = {
    gold: '#D4AF37',
    crimson: '#A4133C',
    lightText: '#E5E7EB',
...
const Benefits = () => {
  ...
  const clipPathValue = useTransform(scrollYProgress, [0, 0.8], ["circle(0% at 50% 50%)", "circle(150% at 50% 50%)"]);
  ...
  return (
    ...
    >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 lg:grid lg:grid-cols-2 lg:gap-x-12 lg:items-start">
        ...
        <div className="lg:col-span-1 mt-12 lg:mt-0">
           ...
           <div className="mb-12 md:mb-16 lg:mb-20 text-center lg:text-left">
            <h2 className={`text-lg font-semibold text-[${colors.crimson}] tracking-wide uppercase`}> {/* Increased from text-base */}
              Value Proposition
            </h2>
            <p className={`mt-2 text-4xl font-extrabold text-[${colors.gold}] sm:text-5xl tracking-tight`}> {/* Increased from 3xl/4xl */}
              Unlock Key Benefits
            </p>
            <p className={`mt-4 max-w-2xl text-xl text-[${colors.mediumText}] lg:mx-0 sm:text-2xl`}> {/* Increased from xl */}
              See how Cold Eye Review transforms field operations and adds value to your business.
            </p>
          </div>
          {/* Benefits List */}
          <div className="space-y-10 lg:space-y-12">
            {benefitsData.map((benefit) => (
              ...
              >
                 {/* Adjusted Text Sizes */}
                <h3 className={`text-2xl font-semibold text-[${colors.gold}] mb-3`}> {/* Increased from xl, added mb-3 */}
                  {benefit.title}
                </h3>
                <p className={`text-lg text-[${colors.lightText}] leading-relaxed`}> {/* Increased from text-base, added leading-relaxed */}
                  {benefit.description}
                </p>
              </motion.div>
            ))}
      ...
      </div> {/* End of max-width container / grid */}
